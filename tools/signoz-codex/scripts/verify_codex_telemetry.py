...
def run_compose(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(compose_args(*args), check=True, text=True, capture_output=True, env=compose_environment())
...
def main() -> int:
    ...
    metrics_count = query_clickhouse(
        ...
        WHERE ts.resource_attrs['service.name'] = {service}
    ...
    print_rows("Approval Outcomes", approval_rows, ["approved", "requests"], "no approval prompts in the selected window")
    ...
    print_rows("Top Websocket Event Kinds", ws_rows, ["kind", "events"], "no websocket event metrics in the selected window")
    ...
    print_rows("Top Log Event Names", log_event_rows, ["event_name", "rows"], "no native Codex logs in the selected window")
    ...
    metric_project_rows = parse_tsv(
        query_clickhouse(
            ...
            SELECT
              ...
              if(ts.resource_attrs['vcs.repository.name'] = '', '-', ts.resource_attrs['vcs.repository.name']) AS repo_name,
    ...
    if metric_project_rows:
        print_rows("Metric Project Dimensions", metric_project_rows, ["project_name", "project_path", "repo_name", "series"])
    else:
        ...
        print(f"  {YELLOW}if you want project-aware native metrics, launch plain codex from a shell that sets project.name/project.path OTEL attrs{RESET}")
    ...
    if project_rows:
        ...
        if metric_project_rows:
            print(f"  {YELLOW}trace cwd is still useful for debugging, but project.name/project.path are the preferred native metric dimensions{RESET}")
        else:
